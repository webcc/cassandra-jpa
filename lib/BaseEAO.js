...
let CassandraTable = require("./CassandraTable");
let CassandraRowInterceptor = require("./CassandraRowInterceptor");
...
module.exports = class BaseEAO {
    constructor(config)
    {
        ...
        this.rowInterceptor = config.rowInterceptor || new CassandraRowInterceptor();
        this.entityClass = CassandraEntity;
        this.baseEntityClass = config.baseEntityClass || CassandraEntity;
        this.table = new CassandraTable(this.client.options.keyspace, {
            name: "base",
    ...
    {
        const queryObject = this.table.buildInsertQueryObject(row);
    ...
    persistAllRows(rows, callback)
    {
        ...
        {
            const queryObject = self.table.buildInsertQueryObject(row);
    ...
    {
        const queryObject = this.table.buildUpdateQueryByCriteria(row, criteria);
        return this.query(queryObject, callback);
    }
    ...
    updateRow(row, callback)
    {
        ...
        /*
         const queryObject = this.table.buildUpdateQuery(row);
         return this.query(queryObject, callback);
         */
    }
    ...
    {
        const queryObject = this.table.buildRemoveQuery(criteria);
        return this.query(queryObject, callback);
    }

    findOneRow(criteria, callback)
    {
        const queryObject = this.table.buildSelectQueryObjectFromCriteria(criteria);
    ...
    toRow(entity)
    {
        ...
        Object.keys(entityClone).forEach(propertyKey =>
        {
            ...
            let type = this.table.fields.get(propertyKey.substring(1));
            entityClone[propertyKey] = this.rowInterceptor.toRow(type, property, this.baseEntityClass);
        });
    ...
    {
        return S(query).template(_.extend(this.getDefaultStringTemplateValues(),
        stringTemplateValues)).s;
    }
    ...
    createTable(callback)
    {
        let self = this;
        let queryObject = {
            query: this.table.buildCreateTableQuery(),
            params: []
        };
        this.query(queryObject, function (error, result)
        {
    ...
    insertIndexes(callback)
    {
        ...
        {
            let queryObject = {
                query: this.table.buildInsertIndexQuery(field),
                params: []
            };
    ...
    dropIndexes()
    {
        ...
        {
            let queryObject = {
                query: this.table.buildDropIndexQuery(field),
                params: []
            };
            indexQueries.push(queryObject);
        });
    ...
    {
        let queryObject = {
            query: this.table.buildDropTableQuery(),
            params: []
        };
        this.query(queryObject, function (error, result)
        {
